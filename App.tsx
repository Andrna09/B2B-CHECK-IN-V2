...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (view) {
      // Public / Driver Views (FULL SCREEN)
  ...
  // --- MAIN RETURN (PERBAIKAN LOGIKA SIDEBAR) ---
  return (
    <>
        {/* 1. Loading Screen */}
        {isTransitioning && (
        ...
        {/* 3. Logic Utama Tampilan */}
        {view === 'home' ? (
            // Jika Home -> Tampilkan Landing Page
            <LandingPage />
        ) : (
            // Jika Halaman Lain -> Cek apakah perlu Sidebar?
            // HANYA jika view adalah 'admin-dashboard' atau 'admin-reports' -> PAKAI SIDEBAR
            ['admin-dashboard', 'admin-reports', 'settings'].includes(view) ? (
               ...
               // SISANYA (Checkin, Status, Security, dll) -> LAYAR PENUH (TANPA SIDEBAR)
