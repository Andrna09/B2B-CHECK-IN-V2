...
const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto space-y-8 bg-slate-50 min-h-screen font-sans">
      ...
      {/* --- HEADER (JUDUL + LOGOUT + REFRESH) --- */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-1 flex flex-col justify-between">
          <div>
            <h1 className="text-3xl font-black text-slate-800 tracking-tight">Dashboard</h1>
            <p className="text-slate-500 font-medium text-sm">Traffic & Warehouse Monitor</p>
          </div>
          
          <div className="flex items-center gap-3 mt-4">
             {/* Indikator Waktu & Refresh */}
             <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 bg-white px-3 py-2 rounded-lg border border-slate-200 shadow-sm">
                 <Clock className="w-3 h-3" />
                 {lastUpdated.toLocaleTimeString()}
                 <button onClick={fetchData} className={`ml-2 hover:text-blue-600 ${loading ? 'animate-spin' : ''}`} title="Refresh Data">
             ...
             <button 
                ...
                className="flex items-center gap-2 px-3 py-2 bg-red-50 text-red-600 border border-red-100 rounded-lg text-[10px] font-bold hover:bg-red-100 transition-colors"
...
const ActionButtons = ({ driver, filter, onRefresh }: any) => {
    ...
    // 2. Verifikasi Action (Security Area)
